...
def inizializza_vector_db():
    print("1. Scarico il dataset da Hugging Face...")
    dataset = load_dataset("KanuSaru/italian-recipes", split="train")
    campione_dev = dataset.select(range(500))
    
    print("2. Trasformo le ricette in 'Documenti' per LangChain...")
    documenti_langchain = []
    ...
    for ricetta in campione_dev:
        ...
        testo_da_vettorizzare = f"Titolo piatto: {titolo_ricetta}."
        testo_ricetta = (
    ...
    print("3. Vettorizzo e salvo nel database Chroma...")
    vector_store = Chroma.from_documents(
        documents=documenti_langchain,
        embedding=EMBEDDINGS,  
        persist_directory="./chroma_db_cucina"
    )
